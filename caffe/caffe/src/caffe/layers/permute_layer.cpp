...
void PermuteLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  PermuteParameter permute_param = this->layer_param_.permute_param(); // 生成PermuteParameter对象（proto中定义）
...
void PermuteLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < num_axes_; ++i) {
    if (i == num_axes_ - 1) {
      ...
      old_steps_.mutable_cpu_data()[i] = bottom[0]->count(i + 1); //  old_steps_ bottom 第i个轴以后的元素总数
    }
  ...
  for (int i = 0; i < num_axes_; ++i) {
    if (i == num_axes_ - 1) {
      ...
      new_steps_.mutable_cpu_data()[i] = top[0]->count(i + 1); //  new_steps_ top 第i个轴以后的元素总数
    }
  }
}
