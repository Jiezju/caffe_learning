...
/*
以输入shape [10,1,12]  indicator [10, 1]  hidden 为3  为例
*/
...
void LstmLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,//bottom[0]: [T]x[N]x[Channels]
  ...
  if_tensorflow_ = this->layer_param_.lstm_param().if_tensorflow();
  ...
  if (this->blobs_.size() > 0) {
    ...
    // Intialize the weight  shape: [12,12]
    ...
    // If necessary, intiialize and fill the bias term  shape [1,12]
...
void LstmLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,//bottom[0]: [T]x[N]x[Channels]
  ...
  // Figure out the dimensions
	...
	N_ = bottom[0]->shape(1);// bottom[0]->num() / N_; 
...
//     "incompatible with inner product parameters.";
  ...
  // Gate initialization 门的shape 计算 [10,1,4,3]
  vector<int> gate_shape;
  ...
  // Set up the bias multiplier  shape [1*10]  初始化为1
  vector<int> multiplier_shape(1, N_*T_); // 元素个数为1 ，值为N_*T_
...
void LstmLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (clip) {
	// 拷贝 将 c_T_ 复制到 c_0_  将 h_T_ 复制到 h_0_   
  ...
  //   pre_gate_data[T_*N_, 4*H_] = bottom_data[T_*N_,I_] * weight_i[I_,4*H_]  由于weight 存的是 [4*H_, I_]反过来的 所以需要对weight进行转置
  caffe_cpu_gemm(CblasNoTrans, CblasTrans, T_*N_, 4*H_, I_, Dtype(1.),
      bottom_data, weight_i, Dtype(0.), pre_gate_data);
  // pre_gate_data = (bias_multiplier_[ T_*N_,1] * bias[1, 4*H_]) + pre_gate_data  (bias_multiplier_ 初始化为1， 所以前面的乘法无效，只是做了广播)
  caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, T_*N_, 4*H_, 1, Dtype(1.),
      bias_multiplier_.cpu_data(), bias, Dtype(1.), pre_gate_data);

  // Compute recurrent forward propagation 遍历每个sequence
  for (int t = 0; t < T_; ++t) {

    // 获取输出内存
    Dtype* h_t = top_data + top_.offset(t);                   //[T]x[N]x[H]     h_t[N,H]
    Dtype* c_t = cell_data + cell_.offset(t);                 //[T]x[N]x[H]     c_t[N,H]

    Dtype* pre_gate_t = pre_gate_data + pre_gate_.offset(t);  // pre_gate_t[N,4,H]
    Dtype* gate_t = gate_data + gate_.offset(t);              // gate_t[N,4,H]
    Dtype* h_to_gate_t = h_to_gate;                           // gate_t[N,4,H] h_to_gate 初始化为0


    // 获取上一时刻的输出
    const Dtype* clip_t = clip ? clip + bottom[1]->offset(t) : NULL; // 更新到下一个 seq 对应的clip值(indicator)
    const Dtype* h_t_1 = t > 0 ? (h_t - top_.offset(1)) : h_0_.cpu_data(); // 获取上一个输出 h_t-1 (top[t-1]) 
    const Dtype* c_t_1 = t > 0 ? (c_t - cell_.offset(1)) : c_0_.cpu_data(); // 获取上一个输出 c_t-1 (cell[t-1]) 

    // Hidden-to-hidden propagation 遍历每个sequence
	  // h_to_gate[N_, 4*H_] = h_t_1[N_,H_] * weight_h[H_,4*H_] 存放的weight是 [4*H_,H_]
    caffe_cpu_gemm(CblasNoTrans, CblasTrans, N_, 4*H_, H_, Dtype(1.), 
        h_t_1, weight_h, Dtype(0.), h_to_gate);

    // 对每条seq[N, H] 进行计算
    for (int n = 0; n < N_; ++n) {
      const bool cont = clip_t ? clip_t[n] : t > 0; // 默认是true 否则去indicator的值， 非0表示true 否则为false
      if (cont) {
        // pre_gate_t += h_to_gate
      ...
      for (int d = 0; d < H_; ++d) {
        ...
        {
            gate_t[d] = sigmoid(pre_gate_t[d]);
            gate_t[H_ + d] = tanh(pre_gate_t[H_ + d]);
            gate_t[2*H_ + d] = cont ? sigmoid(pre_gate_t[2*H_ + d] + 1.0) : Dtype(0.);
            gate_t[3*H_ + d] = sigmoid(pre_gate_t[3*H_ + d]);

            // Compute cell : c(t) = f(t)*c(t-1) + i(t)*g(t)
            c_t[d] = gate_t[2 * H_ + d] * c_t_1[d] + gate_t[d] * gate_t[H_ + d];
            h_t[d] = gate_t[3*H_ + d] * tanh(c_t[d]);
        }
  ...
  for (int t = T_-1; t >= 0; --t) {
    ...
    for (int n = 0; n < N_; ++n) {
      ...
      if (clipping_threshold_ > Dtype(0.)) {
		  caffe_bound<Dtype>(4 * H_, pre_gate_diff_t, -clipping_threshold_,
